# README

TSC '26*

- Working notes on Bash + Sed + Awk are in [bash-bash.md](./bash/bash-bash.md).
    - Update, some more info on read that the explanation (during the TSC) was lacking (Thanks @SahilDeo)
    - You can infact do [arithmetic in sed](https://www.gnu.org/software/sed/manual/html_node/Increment-a-number.html), and it is [turing-complete](https://catonmat.net/proof-that-sed-is-turing-complete). (Thanks @Ansh)
    - A sample problem: log parsing haha: is implemented in [`log_parser.sh`](./bash/log_parser.sh) with sample [`test.log`](./bash/test.log) for you to learn what idiomatic bash scripting looks like.

- Some git explanation in [git.md](./bash/git.md), basically a decent summary of practical commands AND a humane treatment of the diff output syntax.

- Find some GDB problems in [this README](./gdb/ERADME.md). Try for fun!

- Try some fun functional prog problems (originally in Bash but try Python as well! Lambdas, closures and late evaluation!) in [vrbo](./vrbo.html).

- Some more (coz you guys wanted) numpy problems adapted from CS240 (aimless) Labs

- Ansh's Python slides [upcoming]
- Shanmathi's Make slides [upcoming]

---

If you're bored solve the [cse-bingo](./cse_bingo.png) (credit: TK).
