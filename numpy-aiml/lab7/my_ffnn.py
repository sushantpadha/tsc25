...
class FeedForwardNN:
    ...
    def forward_propagation(self, X: np.ndarray):
        ...
        for l in range(self.L):
            a_prev = self.activations[-1]
            z = np.dot(a_prev, self.weights[l]) + self.biases[l]
            self.pre_activations.append(z)
            
            act_type = self.hidden_act if l < self.L-1 else self.output_act
            
            a = self.activate(z, act_type)
            self.activations.append(a)

        return self.activations[-1].copy()
    ...
    def backward_propagation(self, grads: np.ndarray):
        ...
        for l in reversed(range(self.L)):
            ...
            z      = self.pre_activations[l]  # shape: (N, layer_sizes[l+1])
            a_prev = self.activations[l]      # shape: (N, layer_sizes[l])
            
            # convert grads to delta
            act = self.hidden_act if l < self.L-1 else self.output_act
            # magic of dict
            delta = self._act_derivative[act](z, self.activations[l+1], grads)
            
            grad_weights[l] += a_prev.T @ delta # (layer_sizes[l], layer_sizes[l+1])
            grad_biases[l]  += np.sum(delta, axis=0, keepdims=True) # (1, layer_sizes[l+1])
            
            grads = delta @ self.weights[l].T  # (N, layer_sizes[l])
