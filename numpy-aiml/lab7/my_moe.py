...
class MoE:
    ...
    def backward(self, grad_output: np.ndarray):
        ...
        for k in range(self.num_experts):
            expert_grad_output = grad_output * self.gates[:, k:k+1]
            assert expert_grad_output.shape == (grad_output.shape[0], self.expert_outputs.shape[2])
            grads = self.experts[k].backward_propagation(expert_grad_output)
            self.experts[k].update_parameters(*grads)
            
        ## router grads
        # dL/dg = grad_output * f(x) + lam*(1+log(g(x)))  shape=(N,1,D)*(N,K,D).sum(axis=2) + (N,K)
        # i want to get shape = (N, K) -- (Num inputs, Num experts)
        mse_grad = np.sum(grad_output[:, None, :] * self.expert_outputs, axis=2)
        entropy_grad = self.router_reg * (1 + np.log(self.gates + 1e-10)) # N,K
